...
export default function Participants({
	...
	const countWinner = async () => {
		...
		participants.forEach(async (p, index) => {
			{
				let count = await instance.methods
					.getVoteCount(session.id, p.id)
					.call();
	...
	const checkAndAdd = function checkAndAdd(user) {
		...
		if (found === false) {
			...
			if (
				user.approved === true &&
				refParticipants.current.at(index).approved === false
			) {
	...
	useEffect(() => {
		instance.events
			.candidateApproved({ fromBlock: 0, filter: { sessionID: session.id } })
			.on("data", async (evt) => {
				if (parseInt(session.id) === parseInt(evt.returnValues.sessionID)) {
					let candidate = await instance.methods
						.participants(evt.returnValues.candidate)
						.call();
		...
		instance.events
			.voterApproved({ fromBlock: 0, filter: { sessionID: session.id } })
			.on("data", async (evt) => {
				if (parseInt(session.id) === parseInt(evt.returnValues.sessionID)) {
					let voter = await instance.methods
						.participants(evt.returnValues.voter)
						.call();
					voter.type = "voter";
					voter.approved = true;
					checkAndAdd(voter);
		...
		instance.events
			.joinSessionVoterRequest({
				fromBlock: 0,
				filter: { sessionID: parseInt(session.id) },
			})
			.on("data", async (evt) => {
				...
				if (parseInt(session.id) === parseInt(evt.returnValues.sessionID)) {
					...
					switch (result.status) {
						...
						case false:
							let participant = await instance.methods
								.participants(voterAddr)
								.call();
