...
export default function Session({ session, myAddr, instance }) {
	...
	useEffect(() => {
		...
		instance.events
			.voterApproved({
				fromBlock: 0,
				filter: { sessionID: session.id, voter: myAddr },
			})
			.on("data", async (evt) => {
	...
	return (
		<div class="card">
			...
			<div class="voting-info-items">
				<button class={joinVoterClassParams} onClick={joinSessionAsVoter}>
					Join as Voter
				</button>
				<button
					class={joinCandidateClassParams}
					onClick={joinSessionAsCandidate}
				>
					Join as candidate
				</button>
				<button class="btn-form-participants" onClick={showParticipants}>
					Participants
				</button>
			</div>
			...
			>
				<Participants
					...
					checkRegisteredStatus={checkRegisteredStatus}
