...
export default function Sessions({ instance, myAddr }) {
	...
	useEffect(() => {
		const fetchSessions = async () => {
			instance.events
				.sessionCreated({ fromBlock: parseInt(0) })
				.on("data", (evt) => {
					handleSessionCreatd(evt);
				})
	...
	let sessionList = sessions.map((elem, index) => (
		<Session key={index} myAddr={myAddr} instance={instance} session={elem} />
	));
	return <div class="session-list-container">{sessionList}</div>;
}
