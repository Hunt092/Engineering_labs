...
export default function Participant({
			...
			if (participant.approved !== true) {
				...
				switch (participant.type) {
					case "candidate":
						...
						if (voted === false && sessionPhase !== "Locked") {
	...
	const vote = async () => {
		await instance.methods
			.vote(session.id, participant.id)
			.send({ from: myAddr });
		setVoteClassParams("invisible");
		setVoted(true);
	};
	...
	const approve = async () => {
		switch (participant.type) {
			case "voter":
				await instance.methods
					.approveVoter(session.id, participant.id)
					.send({ from: myAddr });
				setApproveClassParams("invisible");
				break;
			case "candidate":
				await instance.methods
					.approveCandidate(session.id, participant.id)
					.send({ from: myAddr });
				setVoteClassParams("vote-btn");
				setApproveClassParams("invisible");
				break;
	...
	return (
		<div class="participant-card">
			...
			<div class="participant-card-body">
				...
				<p class="participant-description">{participant.info}</p>
				<div class="participant-voting-btns">
					<button href="#" class={voteClassParams} onClick={vote}>
						Vote
					</button>
					<button href="#" class={approveClassParams} onClick={approve}>
						Approve {participant.type}
					</button>
